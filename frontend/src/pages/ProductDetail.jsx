  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        setProduct(data);
        const reviewsRes = await fetch(`/api/reviews/product/${id}`, { cache: 'no-store' });
      ...
      <div className="product-detail">
        ...
        <div className="detail-info">
          ...
          <div style={{ marginTop: '30px' }}>
            <h4 style={{ color: '#fff' }}>Customer Reviews</h4>
            {reviews.length === 0 ? <p style={{ color: '#a1a1aa' }}>No reviews yet.</p> : reviews.map((review) => (
              <div key={review._id} style={{ borderTop: '1px solid #27272a', padding: '12px 0' }}>
                <strong>{review.user?.name || 'Customer'}</strong>
                <span className="product-rating"> {'★'.repeat(review.rating)}{'☆'.repeat(5 - review.rating)}</span>
                {review.comment && <p style={{ color: '#a1a1aa' }}>{review.comment}</p>}
              </div>
            ))}
          </div>
